...
//
// i'm making this client-wrapper class an ApplicationScoped component because JaxRS clients
// are quite heavy and i want to reuse the same instance
//
@ApplicationScoped
public class IdentifierAccessor {
	...
	private void initClient() {
		// JaxRS client builder
		client = ClientBuilder.newBuilder()
				.connectTimeout(1, TimeUnit.SECONDS)	// timeouts make the client more resilient
...
//    accessing a list of resources
	public List<String> retrieveCarIdentifications() {
		...
		return response.readEntity(listType)
				.stream()
				.map(CarIdentifier::getIdentifier)
	...
	private Response sendRequest(JsonObject entity) {
		return target.request(MediaType.APPLICATION_JSON_TYPE).post(Entity.json(entity));
	}
